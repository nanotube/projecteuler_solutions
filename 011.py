...
for line in data:
    for i in range(len(line) - 3):
        prod = reduce(mul, line[i:i+4])
        if prod > maxprod:
            maxprod = prod

for col in range(len(data[0]) - 3):
    for i in range(len(data) - 3):
        prod = reduce(mul, [data[i][col], data[i][col+1], data[i][col+2], data[i][col+3]])
        if prod > maxprod:
            maxprod = prod

for i in range(len(data)-3):
    for j in range(len(data[0]) - 3):
        prod = reduce(mul, [data[i][j], data[i+1][j+1], data[i+2][j+2], data[i+3][j+3]])
        if prod > maxprod:
            maxprod = prod
